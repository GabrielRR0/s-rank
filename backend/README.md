...
- **Bloqueo de tipos de archivo peligrosos** (`services/sharedContent/security/blocked_file_types.py`): rechaza por extensión y por `Content-Type` declarado — ejecutables, scripts, instaladores, documentos de Office con macros. Ver sección 11 para el alcance real (no es un antivirus).
...
**¿Alguien puede compartir algo malicioso a través de este servicio?** Depende de qué tipo de "malicioso":
- **Texto con HTML/`<script>` incrustado**: no es un riesgo — el frontend renderiza el texto revelado con interpolación de Vue (`{{ }}`), que escapa cualquier HTML automáticamente. No se usa `v-html` en ningún lado del proyecto, así que no hay manera de que el texto compartido ejecute código en el navegador de quien lo ve.
- **Un archivo con malware de verdad**: acá sí hay una decisión consciente de alcance. `blocked_file_types.py` (sección 9) rechaza extensiones/`Content-Type` de ejecutables, scripts e instaladores conocidos — frena el caso común ("alguien comparte un `.exe` disfrazado"), pero **no es un antivirus**: no abre ni inspecciona el contenido real del archivo, así que un ejecutable renombrado con otra extensión, o un exploit dentro de un formato aceptado (ej. un PDF/documento con contenido malicioso embebido), no se detecta. Un escaneo real requeriría mandar el archivo en texto plano a un servicio externo *antes* de encriptarlo, lo que contradice la promesa central del proyecto de que nadie más que el destinatario ve el contenido — se optó deliberadamente por no cruzar esa línea (ver `security/README.md`).
- **Abuso del servicio como "hosting" de malware a escala**: el diseño ya limita esto de forma estructural, no como medida dedicada — cada enlace se ve una sola vez (o expira en máximo 24hs) y se autodestruye, a diferencia de un host de archivos público persistente. `RATE_LIMIT_CREATE` (10/min por IP) y el Turnstile opcional (arriba) limitan además cuánto volumen puede generar un solo actor automatizado.
- Como capa adicional fuera de nuestro control pero real: cuando el navegador del destinatario *descarga* el archivo, Safe Browsing (Chrome/Edge) o SmartScreen (Windows) suelen escanear el archivo descargado de forma independiente — no es algo que este proyecto implemente, pero es una red de seguridad adicional que ya existe del lado del destinatario para muchos tipos de malware conocido.
